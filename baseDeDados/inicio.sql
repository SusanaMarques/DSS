...
CREATE TABLE IF NOT EXISTS `MC`.`PlaylistMusica` (
  ...
  INDEX `fk_Playlist_Utilizador_idx` (`idUtilizador` ASC) VISIBLE,
  INDEX `fk_Playlist_Conteudo_idx` (`idMusica` ASC) VISIBLE,
  CONSTRAINT `fk_Playlist_Conteudo`
    FOREIGN KEY (`idMusica`)
    REFERENCES `MC`.`Musica` (`idMusica`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Playlist_Utilizador`
...
CREATE TABLE IF NOT EXISTS `MC`.`PlaylistVideo` (
  ...
  PRIMARY KEY (`idPlaylist`),
  INDEX `fk_Playlist_Utilizador_idx` (`idUtilizador` ASC) VISIBLE,
  INDEX `fk_Playlist_Conteudo0_idx` (`idVideo` ASC) VISIBLE,
  CONSTRAINT `fk_Playlist_Conteudo0`
    FOREIGN KEY (`idVideo`)
    REFERENCES `MC`.`Video` (`idVideo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Playlist_Utilizador0`
    FOREIGN KEY (`idUtilizador`)
    REFERENCES `MC`.`UtilizadorRegistado` (`idUtilizador`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
